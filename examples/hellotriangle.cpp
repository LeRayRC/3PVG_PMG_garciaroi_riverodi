...
private:
	...
	void createImageViews() {
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++) {
			...
			if (vkCreateImageView(device, &createInfo,
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile("../src/shaders/hellotrianglevert.spv");
		auto fragShaderCode = readFile("../src/shaders/hellotrianglefrag.spv");
		
		VkShaderModule vertShaderModule = createShaderModule(device, vertShaderCode);
		VkShaderModule fragShaderModule = createShaderModule(device, fragShaderCode);

		//Empezamos a crear el paso de vertex shader
		...
		//Algunas propiedades del pipeline grafico se pueden
		//declarar como dinamicas para luego modificarlas
		//cuando se lanzan los comandos de dibujado
		...
		//Para este ejemplo no hay vertex data ya que está harcoded dentro
		//del shader, en un futuro se volverá a esto
		...
		//DE MOMENTO NO ACTIVAMOS EL CULLING
		...
		//DE MOMENTO SE QUEDA DESACTIVADO EL MULTISAMPLING
		...
		//No hay alpha blending y el nuevo color sustituye al antiguo
		...
		//Indica el indice del subpass asociado al render pass
	...
	void createRenderPass() {
		...
		//Al no hacer multisampling solo necesitamos un sample de momento
