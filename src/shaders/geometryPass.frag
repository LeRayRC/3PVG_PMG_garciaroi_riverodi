...
layout(set = 1, binding = 5) uniform LightBlock{
  LightProperties light[100];
};
...
vec3 DirectionalLight(LightProperties light){
...
void main() 
{
  if(light[0].enabled == 1){
    switch(light[0].type){
      case 0: {
        outFragColor = vec4(DirectionalLight(light[0]),1.0);
        break;
       }
       case 1: {
        break;
