...
LavaImage::LavaImage(LavaEngine* engine,void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped){
	...
	LavaBuffer upload_buffer = LavaBuffer(*engine_->allocator_, data_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
...
LavaImage::LavaImage(LavaEngineVR* engine, void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped) {
	engine_ = nullptr;
	engine_vr_ = engine;
	size_t data_size = size.depth * size.width * size.height * 4;

	LavaBuffer upload_buffer = LavaBuffer(*engine_vr_->allocator_, data_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);

	memcpy(upload_buffer.get_buffer().info.pMappedData, data, data_size);

	allocate(size, format, usage, mipmapped);

	engine_vr_->immediate_submit([&](VkCommandBuffer cmd) {
		TransitionImage(cmd, image_.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	...
	vkCreateSampler(engine_vr_->device_->get_device(), &sampler_info, nullptr, &sampler_);
}
...
LavaImage::LavaImage(LavaEngineVR* engine,
	std::string path,
	VkFormat format,
	VkImageUsageFlags usage,
	bool mipmapped) {
...
LavaImage::~LavaImage(){
	if (engine_) {
		...
		vkDestroySampler(engine_vr_->device_->get_device(), sampler_, nullptr);
		vkDestroyImageView(engine_vr_->device_->get_device(), image_.image_view, nullptr);
		vmaDestroyImage(engine_vr_->allocator_->get_allocator(), image_.image, image_.allocation);
	}
...
void LavaImage::allocate(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped, int layers)
{
	...
	if (engine_) {
		...
		vmaCreateImage(engine_vr_->allocator_->get_allocator(), &img_info, &alloc_info, &image_.image, &image_.allocation, nullptr);
	...
	if (engine_) {
		...
		if (vkCreateImageView(engine_vr_->device_->get_device(), &view_info, nullptr, &image_.image_view) != VK_SUCCESS) {
			printf("Failed to allocate LavaImage\n");
			exit(-1);
		}
	}
